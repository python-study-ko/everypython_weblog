...
class CategoryMixin(object):
    ...
    def navi_bar(self):
        ...
        if c_info:
            ...
            # 아무런 카테고리가 존재하지 않을 경우(블로그 첫 설치했을떄)를 템플릿에서 분기하기 위해 None값으로 반환함
            category_tree = None
...
class Category(models.Model):
    ...
    parent = models.ForeignKey('self', limit_choices_to=Q(level=1) | Q(level=2), on_delete=models.CASCADE, blank=True,
                               null=True, help_text="상위 카테고리를 정해주세요,미지정시 최상위 카테고리가 됩니", verbose_name='상위 카테고리')
...
class PublishMixin(object):
    ...
    def posts_info(self, post_queryset):
        ...
        info_list = [(
                     p.pk, p.category.name, p.create_date, p.title, p.description if p.description else p.content[:100],
                     [t.name for t in p.tag.all()]) for p in posts]
        return info_list
...
class Post(models.Model, HitCountMixin):
    ...
    description = models.CharField(verbose_name='요약', max_length=160, blank=True, null=True,
    ...
    publish = models.BooleanField(verbose_name='발행여부', default=False,
                                  help_text="포스트 발행시 외부에 포스트가 공개됩니다. 만약 발행하지 않는다면 작성자만 해당 포스트를 볼 수 있습니다.")
